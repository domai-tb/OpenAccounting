...
The application SHALL retain the existing GetIt/AppScope/AppServices dependency flow and use Flutter-native state mechanisms for page state. Caching and invalidation MAY be implemented by the existing services or an equivalent state layer; Riverpod is not required. Optimistic updates SHALL be limited to idempotent operations. Finalization, payment, stock, and other transactional mutations SHALL commit atomically; a retryable mutation MUST carry a unique idempotency key and repeated requests with that key MUST have one effect.
...
THEN fresh data SHALL be fetched from the repository or data source
...
AND the active data source returns a 422 validation error within 3 seconds
AND the payment mutation carries a unique idempotency key
THEN the UI SHALL revert to the previous unpaid state
AND an error snackbar SHALL display the data-source validation message

#### Scenario: Optimistic Update Success

GIVEN an invoice is currently marked as unpaid
WHEN the user marks the invoice as paid
AND the idempotent payment mutation returns success
