...
- [ ] 2.1 Write failing test: `test/app/app_shell_test.dart` — expanded at 1280, compact rail at 1024, drawer at 800, collapses and persists, highlights correctly (assert fails)
...
- [ ] 3.1 Write failing test: `test/design_system/app_page_header_test.dart` — header shows title+primary, filter toolbar with chips+count, subtitle without primary (assert fails)
- [ ] 3.2 Implement: `lib/design_system/components/app_page_header.dart` with title/subtitle/primary action right, tabs, filter/search toolbar per §5
