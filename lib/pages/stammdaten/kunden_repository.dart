import 'package:drift/drift.dart';

class KundenException implements Exception {
  const KundenException(this.message);

  final String message;

  @override
  String toString() => message;
}

class Kunde {
  const Kunde({
    required this.id,
    required this.debitorNr,
    required this.anrede,
    required this.name,
    this.firma,
    required this.strasse,
    this.hausnummer,
    required this.plz,
    required this.ort,
    required this.land,
    this.ustIdNr,
    this.foreignTaxNumber,
    this.telefon,
    this.email,
    required this.zahlungsziel,
    required this.skontoProzent,
    required this.skontoTage,
    this.kreditlimit,
    required this.dunningBlocked,
    this.dunningBlockedUntil,
    this.dunningBlockedReason,
    required this.zugferdAktiv,
    this.note,
  });

  final int id;
  final String debitorNr;
  final String anrede;
  final String name;
  final String? firma;
  final String strasse;
  final String? hausnummer;
  final String plz;
  final String ort;
  final String land;
  final String? ustIdNr;
  final String? foreignTaxNumber;
  final String? telefon;
  final String? email;
  final int zahlungsziel;
  final num skontoProzent;
  final int skontoTage;
  final num? kreditlimit;
  final bool dunningBlocked;
  final String? dunningBlockedUntil;
  final String? dunningBlockedReason;
  final bool zugferdAktiv;
  final String? note;

  String get kundennummer => debitorNr;

  String? get steuernummerAusland => foreignTaxNumber;

  bool get mahngesperrt => dunningBlocked;
}

class CreditLimitWarning {
  const CreditLimitWarning({
    required this.outstanding,
    required this.limit,
    required this.requiresConfirmation,
    required this.auditLogged,
  });

  final num outstanding;
  final num limit;
  final bool requiresConfirmation;
  final bool auditLogged;
}

class CreditLimitConfirmation {
  const CreditLimitConfirmation({required this.status, this.warning});

  final String status;
  final CreditLimitWarning? warning;
}

typedef CreditLimitConfirmationResult = CreditLimitConfirmation;

class DunningRunResult {
  DunningRunResult({required List<int> skippedCustomerIds, required Map<int, String> skippedReasons})
    : skippedCustomerIds = List<int>.unmodifiable(List<int>.from(skippedCustomerIds)),
      skippedReasons = Map<int, String>.unmodifiable(Map<int, String>.from(skippedReasons));

  final List<int> skippedCustomerIds;
  final Map<int, String> skippedReasons;
}

class InvoiceOptions {
  const InvoiceOptions({required this.zugferdEnabled, required this.foreignTaxNumber});

  final bool zugferdEnabled;
  final String? foreignTaxNumber;

  bool get zugferdAktiv => zugferdEnabled;

  String? get steuernummerAusland => foreignTaxNumber;
}

class KundenRepository {
  KundenRepository(this.executor);

  final QueryExecutor executor;
  Future<void>? _schemaReady;

  Future<void> ensureSchema() => _schemaReady ??= _ensureSchema(executor);

  static const List<_ColumnDefinition> _kundenColumns = <_ColumnDefinition>[
    _ColumnDefinition('kundennummer', 'TEXT'),
    _ColumnDefinition('anrede', "TEXT NOT NULL DEFAULT 'Herr'"),
    _ColumnDefinition('hausnummer', 'TEXT'),
    _ColumnDefinition('steuernummer_ausland', 'VARCHAR(50)'),
    _ColumnDefinition('debitor_nr', 'TEXT'),
    _ColumnDefinition('kreditlimit', 'NUMERIC(12,2)'),
    _ColumnDefinition('mahngesperrt', 'INTEGER NOT NULL DEFAULT 0'),
    _ColumnDefinition('mahngesperrt_bis', 'TEXT'),
    _ColumnDefinition('mahngesperrt_grund', 'TEXT'),
    _ColumnDefinition('zugferd_aktiv', 'INTEGER NOT NULL DEFAULT 0'),
    _ColumnDefinition('skonto_tage', 'INTEGER NOT NULL DEFAULT 0'),
    _ColumnDefinition('note', 'TEXT'),
    _ColumnDefinition('notiz', 'TEXT'),
  ];

  static const List<_ColumnDefinition> _nummernkreisColumns = <_ColumnDefinition>[
    _ColumnDefinition('naechste_nummer', 'INTEGER DEFAULT 1'),
    _ColumnDefinition('aktiv', 'INTEGER NOT NULL DEFAULT 1'),
    _ColumnDefinition('letzte_nummer', 'INTEGER'),
    _ColumnDefinition('letze_nummer', 'INTEGER'),
  ];

  static const Map<String, String> _updateColumns = <String, String>{
    'anrede': 'anrede',
    'name': 'name',
    'firma': 'firma',
    'strasse': 'strasse',
    'hausnummer': 'hausnummer',
    'plz': 'plz',
    'ort': 'ort',
    'land': 'land',
    'ustIdNr': 'ust_idnr',
    'ust_idnr': 'ust_idnr',
    'foreignTaxNumber': 'steuernummer_ausland',
    'steuernummerAusland': 'steuernummer_ausland',
    'steuernummer_ausland': 'steuernummer_ausland',
    'telefon': 'telefon',
    'email': 'email',
    'zahlungsziel': 'zahlungsziel',
    'skontoProzent': 'skonto_prozent',
    'skonto_prozent': 'skonto_prozent',
    'skontoTage': 'skonto_tage',
    'skonto_tage': 'skonto_tage',
    'kreditlimit': 'kreditlimit',
    'dunningBlocked': 'mahngesperrt',
    'mahngesperrt': 'mahngesperrt',
    'dunningBlockedUntil': 'mahngesperrt_bis',
    'mahngesperrtBis': 'mahngesperrt_bis',
    'mahngesperrt_bis': 'mahngesperrt_bis',
    'dunningBlockedReason': 'mahngesperrt_grund',
    'mahngesperrtGrund': 'mahngesperrt_grund',
    'mahngesperrt_grund': 'mahngesperrt_grund',
    'zugferdAktiv': 'zugferd_aktiv',
    'zugferd_aktiv': 'zugferd_aktiv',
    'note': 'note',
    'notiz': 'note',
  };

  static const Set<String> _booleanColumns = <String>{'mahngesperrt', 'zugferd_aktiv'};

  static const Map<String, String> _fallbackVatPatterns = <String, String>{
    'DE': r'^DE[0-9]{9}$',
    'AT': r'^ATU[0-9]{8}$',
    'FR': r'^FR[0-9A-Z]{2}[0-9]{9}$',
    'IT': r'^IT[0-9]{11}$',
    'ES': r'^ES[A-Z0-9][0-9]{7}[A-Z0-9]$',
    'NL': r'^NL[0-9]{9}B[0-9]{2}$',
    'PL': r'^PL[0-9]{10}$',
    'BE': r'^BE[0-9]{10}$',
    'SE': r'^SE[0-9]{12}$',
    'DK': r'^DK[0-9]{8}$',
    'FI': r'^FI[0-9]{8}$',
    'IE': r'^IE[0-9][0-9A-Z*+][0-9]{5}[A-Z]{1,2}$',
    'PT': r'^PT[0-9]{9}$',
    'CZ': r'^CZ[0-9]{8,10}$',
    'HR': r'^HR[0-9]{11}$',
    'HU': r'^HU[0-9]{8}$',
    'RO': r'^RO[0-9]{2,10}$',
    'BG': r'^BG[0-9]{9,10}$',
    'SK': r'^SK[0-9]{10}$',
    'SI': r'^SI[0-9]{8}$',
    'LT': r'^LT[0-9]{9,12}$',
    'LV': r'^LV[0-9]{11}$',
    'EE': r'^EE[0-9]{9}$',
    'CY': r'^CY[0-9]{8}[A-Z]$',
    'MT': r'^MT[0-9]{8}$',
    'LU': r'^LU[0-9]{8}$',
    'GR': r'^EL[0-9]{9}$',
  };

  static const String _customerSelect = '''
SELECT id, kundennummer, debitor_nr, anrede, name, firma, strasse, hausnummer,
       plz, ort, land, ust_idnr, steuernummer_ausland, telefon, email,
       zahlungsziel, skonto_prozent, skonto_tage, kreditlimit, mahngesperrt,
       mahngesperrt_bis, mahngesperrt_grund, zugferd_aktiv, note, notiz
FROM kunden
''';

  Future<Kunde> create({
    required String name,
    String anrede = 'Herr',
    String? firma,
    required String strasse,
    String? hausnummer,
    required String plz,
    required String ort,
    String land = 'DE',
    String? ustIdNr,
    String? foreignTaxNumber,
    String? telefon,
    String? email,
    int zahlungsziel = 14,
    num skontoProzent = 0,
    int skontoTage = 0,
    num? kreditlimit,
    bool dunningBlocked = false,
    String? dunningBlockedUntil,
    String? dunningBlockedReason,
    bool zugferdAktiv = false,
    String? note,
  }) async {
    _validateRequired(anrede, 'Anrede');
    _validateRequired(name, 'Name');
    _validateRequired(strasse, 'Straße');
    _validateRequired(plz, 'PLZ');
    _validateRequired(ort, 'Ort');
    _validateRequired(land, 'Land');
    final country = _normalizeCountry(land);
    _validateForeignTaxNumber(foreignTaxNumber);
    await ensureSchema();
    await _validateVatId(country, ustIdNr);

    final transaction = executor.beginTransaction();
    late final int id;
    try {
      await transaction.ensureOpen(_NoopTransactionUser());
      final debitorNr = await _allocateDebitorNumber(transaction);
      id = await transaction.runInsert(
        '''
INSERT INTO kunden (
  kundennummer, debitor_nr, anrede, name, firma, strasse, hausnummer, plz, ort, land,
  ust_idnr, steuernummer_ausland, telefon, email, zahlungsziel, skonto_prozent, skonto_tage,
  kreditlimit, mahngesperrt, mahngesperrt_bis, mahngesperrt_grund, zugferd_aktiv, note, notiz
)
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
''',
        <Object?>[
          debitorNr,
          debitorNr,
          anrede,
          name,
          firma,
          strasse,
          hausnummer,
          plz,
          ort,
          country,
          ustIdNr,
          foreignTaxNumber,
          telefon,
          email,
          zahlungsziel,
          skontoProzent,
          skontoTage,
          kreditlimit,
          if (dunningBlocked) 1 else 0,
          dunningBlockedUntil,
          dunningBlockedReason,
          if (zugferdAktiv) 1 else 0,
          note,
          note,
        ],
      );
      await transaction.send();
    } catch (error, stackTrace) {
      await _rollback(transaction);
      if (error is KundenException) {
        Error.throwWithStackTrace(error, stackTrace);
      }
      Error.throwWithStackTrace(const KundenException('Kunde konnte nicht gespeichert werden'), stackTrace);
    }

    final stored = await findById(id);
    if (stored == null) {
      throw const KundenException('Kunde konnte nicht gespeichert werden');
    }
    return stored;
  }

  Future<Kunde?> findById(int id) async {
    await ensureSchema();
    final rows = await executor.runSelect('$_customerSelect WHERE id = ?', <Object?>[id]);
    return rows.isEmpty ? null : _kundeFromRow(rows.single);
  }

  Future<List<Kunde>> list() async {
    await ensureSchema();
    final rows = await executor.runSelect('$_customerSelect ORDER BY id', const <Object?>[]);
    return rows.map(_kundeFromRow).toList(growable: false);
  }

  Future<Kunde> update(int id, Map<String, dynamic> values) async {
    await ensureSchema();
    final currentRows = await executor.runSelect('$_customerSelect WHERE id = ?', <Object?>[id]);
    if (currentRows.isEmpty) {
      throw const KundenException('Kunde nicht gefunden');
    }

    final current = currentRows.single;
    final merged = Map<String, Object?>.from(current);
    final assignments = <String, Object?>{};
    for (final entry in values.entries) {
      final column = _updateColumns[entry.key];
      if (column == null) {
        throw KundenException('Unbekanntes Kundenfeld: ${entry.key}');
      }
      final value = _toDatabaseValue(column, entry.value);
      assignments[column] = value;
      merged[column] = value;
    }
    if (assignments.isEmpty) return _kundeFromRow(current);

    _validateRequired(_asString(merged['anrede']) ?? '', 'Anrede');
    _validateRequired(_asString(merged['name']) ?? '', 'Name');
    _validateRequired(_asString(merged['strasse']) ?? '', 'Straße');
    _validateRequired(_asString(merged['plz']) ?? '', 'PLZ');
    _validateRequired(_asString(merged['ort']) ?? '', 'Ort');
    _validateRequired(_asString(merged['land']) ?? '', 'Land');
    _validateForeignTaxNumber(_asString(merged['steuernummer_ausland']));
    await _validateVatId(_normalizeCountry(_asString(merged['land']) ?? 'DE'), _asString(merged['ust_idnr']));

    final assignmentSql = assignments.keys.map((column) => '$column = ?').join(', ');
    try {
      await executor.runUpdate('UPDATE kunden SET $assignmentSql WHERE id = ?', <Object?>[...assignments.values, id]);
    } catch (error, stackTrace) {
      Error.throwWithStackTrace(const KundenException('Kunde konnte nicht aktualisiert werden'), stackTrace);
    }

    final updated = await findById(id);
    if (updated == null) {
      throw const KundenException('Kunde konnte nicht aktualisiert werden');
    }
    return updated;
  }

  Future<void> delete(int id) async {
    await ensureSchema();
    final transaction = executor.beginTransaction();
    try {
      await transaction.ensureOpen(_NoopTransactionUser());
      final reference = await _customerReference(transaction, id);
      if (reference != null) {
        throw KundenException('Kunde kann nicht gelöscht werden: $reference');
      }

      final deleted = await transaction.runDelete('DELETE FROM kunden WHERE id = ?', <Object?>[id]);
      if (deleted == 0) {
        throw const KundenException('Kunde nicht gefunden');
      }
      await transaction.send();
    } catch (error, stackTrace) {
      await _rollback(transaction);
      if (error is KundenException) {
        Error.throwWithStackTrace(error, stackTrace);
      }
      Error.throwWithStackTrace(const KundenException('Kunde konnte nicht gelöscht werden'), stackTrace);
    }
  }

  Future<CreditLimitWarning?> checkCreditLimit({required int customerId, required num invoiceTotal}) async {
    await ensureSchema();
    final customerRows = await executor.runSelect('SELECT kreditlimit FROM kunden WHERE id = ?', <Object?>[customerId]);
    if (customerRows.isEmpty) {
      throw const KundenException('Kunde nicht gefunden');
    }

    final limit = _asNum(customerRows.single['kreditlimit']);
    if (limit == null) return null;
    final outstandingRows = await executor.runSelect(
      '''
SELECT COALESCE(SUM(betrag), 0) AS outstanding
FROM forderungen
WHERE kunde_id = ?
  AND COALESCE(status, 'offen') IN ('offen', 'teilbezahlt')
''',
      <Object?>[customerId],
    );
    final outstanding = outstandingRows.isEmpty ? 0 : (_asNum(outstandingRows.single['outstanding']) ?? 0);
    if (_toCents(outstanding) + _toCents(invoiceTotal) <= _toCents(limit)) return null;
    return CreditLimitWarning(
      outstanding: _fromCents(_toCents(outstanding)),
      limit: _fromCents(_toCents(limit)),
      requiresConfirmation: true,
      auditLogged: true,
    );
  }

  Future<CreditLimitConfirmation> confirmCreditLimitWarning({
    required int customerId,
    required num invoiceTotal,
  }) async {
    final warning = await checkCreditLimit(customerId: customerId, invoiceTotal: invoiceTotal);
    return CreditLimitConfirmation(status: 'final', warning: warning);
  }

  Future<DunningRunResult> runDunning() async {
    await ensureSchema();
    final rows = await executor.runSelect('''
SELECT DISTINCT k.id, k.mahngesperrt, k.mahngesperrt_bis
FROM kunden k
JOIN forderungen f ON f.kunde_id = k.id
WHERE COALESCE(f.status, 'offen') IN ('offen', 'teilbezahlt')
  AND f.faelligkeit IS NOT NULL
  AND date(f.faelligkeit) <= date('now')
ORDER BY k.id
''', const <Object?>[]);
    final skippedIds = <int>[];
    final skippedReasons = <int, String>{};
    for (final row in rows) {
      if (_isDunningBlocked(row)) {
        final id = _asInt(row['id']);
        if (id != null) {
          skippedIds.add(id);
          skippedReasons[id] = 'Mahngesperrt';
        }
      }
    }
    return DunningRunResult(skippedCustomerIds: skippedIds, skippedReasons: skippedReasons);
  }

  Future<void> generateDunningLetter({required int customerId}) async {
    await ensureSchema();
    final rows = await executor.runSelect('SELECT mahngesperrt, mahngesperrt_bis FROM kunden WHERE id = ?', <Object?>[
      customerId,
    ]);
    if (rows.isEmpty) {
      throw const KundenException('Kunde nicht gefunden');
    }
    if (_isDunningBlocked(rows.single)) {
      throw const KundenException('Kunde ist mahngesperrt');
    }
  }

  Future<InvoiceOptions> invoiceOptions({required int customerId}) async {
    await ensureSchema();
    final rows = await executor.runSelect(
      'SELECT zugferd_aktiv, steuernummer_ausland FROM kunden WHERE id = ?',
      <Object?>[customerId],
    );
    if (rows.isEmpty) {
      throw const KundenException('Kunde nicht gefunden');
    }
    final row = rows.single;
    return InvoiceOptions(
      zugferdEnabled: _asBool(row['zugferd_aktiv']),
      foreignTaxNumber: _asString(row['steuernummer_ausland']),
    );
  }

  static Future<void> _ensureSchema(QueryExecutor executor) async {
    await executor.ensureOpen(_NoopTransactionUser());
    await _addMissingColumns(executor, 'kunden', _kundenColumns);
    await _addMissingColumns(executor, 'nummernkreise', _nummernkreisColumns);
    await executor.runCustom(
      'UPDATE kunden SET debitor_nr = kundennummer WHERE debitor_nr IS NULL AND kundennummer IS NOT NULL',
    );
    await executor.runCustom(
      'UPDATE nummernkreise SET letzte_nummer = naechste_nummer - 1 '
      'WHERE letzte_nummer IS NULL AND naechste_nummer IS NOT NULL',
    );
    await executor.runCustom(
      'UPDATE nummernkreise SET letze_nummer = naechste_nummer - 1 '
      'WHERE letze_nummer IS NULL AND naechste_nummer IS NOT NULL',
    );
    await executor.runCustom(
      'CREATE UNIQUE INDEX IF NOT EXISTS idx_kunden_debitor_nr_unique '
      'ON kunden(debitor_nr) WHERE debitor_nr IS NOT NULL',
    );
  }

  static Future<void> _addMissingColumns(
    QueryExecutor executor,
    String table,
    List<_ColumnDefinition> definitions,
  ) async {
    final rows = await executor.runSelect('PRAGMA table_info($table)', const <Object?>[]);
    final existing = <String>{
      for (final row in rows)
        if (row['name'] is String) row['name']! as String,
    };
    for (final definition in definitions) {
      if (!existing.contains(definition.name)) {
        await executor.runCustom('ALTER TABLE $table ADD COLUMN ${definition.name} ${definition.definition}');
      }
    }
  }

  Future<String> _allocateDebitorNumber(TransactionExecutor transaction) async {
    final rows = await transaction.runSelect(
      '''
SELECT id, format, naechste_nummer, letzte_nummer, letze_nummer
FROM nummernkreise
WHERE typ = ? AND aktiv = 1
ORDER BY id
LIMIT 1
''',
      const <Object?>['debitor'],
    );
    if (rows.isEmpty) {
      throw const KundenException('Debitor-Nummernkreis fehlt');
    }

    final row = rows.single;
    final number = _nextNumber(row);
    final updated = await transaction.runUpdate(
      '''
UPDATE nummernkreise
SET naechste_nummer = ?, letzte_nummer = ?, letze_nummer = ?
WHERE id = ?
''',
      <Object?>[number + 1, number, number, row['id']],
    );
    if (updated != 1) {
      throw const KundenException('Debitor-Nummernkreis konnte nicht aktualisiert werden');
    }
    return _formatNumber(_asString(row['format']) ?? '1####', number);
  }

  Future<void> _validateVatId(String country, String? value) async {
    if (value == null) return;
    final rows = await executor.runSelect('SELECT ust_idnr_format FROM eu_laender WHERE laendercode = ?', <Object?>[
      country,
    ]);
    final seededPattern = rows.isEmpty ? null : _asString(rows.single['ust_idnr_format']);
    final pattern = seededPattern?.trim().isNotEmpty == true ? seededPattern : _fallbackVatPatterns[country];
    if (pattern == null) return;

    var valid = false;
    try {
      valid = RegExp(pattern).hasMatch(value);
    } on FormatException {
      valid = false;
    }
    if (!valid) {
      throw KundenException(_vatError(country));
    }
  }

  Kunde _kundeFromRow(Map<String, Object?> row) {
    return Kunde(
      id: _requiredIntValue(row['id'], 'ID'),
      debitorNr: _requiredText(_asString(row['debitor_nr']) ?? _asString(row['kundennummer']), 'Debitor-Nr'),
      anrede: _requiredText(row['anrede'], 'Anrede'),
      name: _requiredText(row['name'], 'Name'),
      firma: _asString(row['firma']),
      strasse: _requiredText(row['strasse'], 'Straße'),
      hausnummer: _asString(row['hausnummer']),
      plz: _requiredText(row['plz'], 'PLZ'),
      ort: _requiredText(row['ort'], 'Ort'),
      land: _requiredText(row['land'], 'Land'),
      ustIdNr: _asString(row['ust_idnr']),
      foreignTaxNumber: _asString(row['steuernummer_ausland']),
      telefon: _asString(row['telefon']),
      email: _asString(row['email']),
      zahlungsziel: _asInt(row['zahlungsziel']) ?? 14,
      skontoProzent: _asNum(row['skonto_prozent']) ?? 0,
      skontoTage: _asInt(row['skonto_tage']) ?? 0,
      kreditlimit: _asNum(row['kreditlimit']),
      dunningBlocked: _asBool(row['mahngesperrt']),
      dunningBlockedUntil: _asString(row['mahngesperrt_bis']),
      dunningBlockedReason: _asString(row['mahngesperrt_grund']),
      zugferdAktiv: _asBool(row['zugferd_aktiv']),
      note: _asString(row['note']) ?? _asString(row['notiz']),
    );
  }

  static int _nextNumber(Map<String, Object?> row) {
    final configured = _asInt(row['naechste_nummer']);
    final lastValues = <int>[
      if (_asInt(row['letzte_nummer']) != null) _asInt(row['letzte_nummer'])!,
      if (_asInt(row['letze_nummer']) != null) _asInt(row['letze_nummer'])!,
    ];
    final last = lastValues.isEmpty ? null : lastValues.reduce((a, b) => a > b ? a : b);
    final minimum = last == null ? 1 : last + 1;
    return (configured ?? 1) > minimum ? (configured ?? 1) : minimum;
  }

  static String _formatNumber(String format, int number) {
    final numericPattern = RegExp(r'^([0-9]*)(#+)$').firstMatch(format);
    if (numericPattern != null) {
      final prefix = numericPattern.group(1)!;
      final width = numericPattern.group(2)!.length;
      final value = number.toString();
      if (prefix.isEmpty) return value.padLeft(width, '0');
      if (value.startsWith(prefix) && value.length > width) return value;
      return '$prefix${value.padLeft(width, '0')}';
    }
    final now = DateTime.now();
    var result = format
        .replaceAll('{YYYY}', now.year.toString())
        .replaceAll('{YY}', _twoDigits(now.year % 100))
        .replaceAll('{MM}', _twoDigits(now.month))
        .replaceAll('YYYY', now.year.toString())
        .replaceAll('YY', _twoDigits(now.year % 100))
        .replaceAll('MM', _twoDigits(now.month));
    result = result.replaceAllMapped(RegExp(r'\{N+\}'), (match) {
      final width = match.group(0)!.length - 2;
      return number.toString().padLeft(width, '0');
    });
    return result.replaceAllMapped(RegExp('#+'), (match) {
      return number.toString().padLeft(match.group(0)!.length, '0');
    });
  }

  static String _twoDigits(int value) => value.toString().padLeft(2, '0');

  static Object? _toDatabaseValue(String column, Object? value) {
    if (_booleanColumns.contains(column)) return _asBool(value) ? 1 : 0;
    if (column == 'land') return _normalizeCountry(_asString(value) ?? 'DE');
    return value;
  }

  static bool _isDunningBlocked(Map<String, Object?> row) {
    if (!_asBool(row['mahngesperrt'])) return false;
    final until = _asString(row['mahngesperrt_bis']);
    if (until == null || until.isEmpty) return true;
    final date = DateTime.tryParse(until);
    return date == null || !DateTime.now().isAfter(date);
  }

  static String _normalizeCountry(String value) => value.trim().toUpperCase();

  static void _validateRequired(String value, String field) {
    if (value.trim().isEmpty) {
      throw KundenException('$field ist Pflicht');
    }
  }

  static String _vatError(String country) {
    switch (country) {
      case 'DE':
        return 'USt-IdNr ungültig: Erwartet DE gefolgt von 9 Ziffern';
      case 'AT':
        return 'USt-IdNr ungültig: Erwartet ATU gefolgt von 8 Ziffern';
      case 'FR':
        return 'USt-IdNr ungültig: Erwartet FR gefolgt von 13 Zeichen';
      default:
        return 'USt-IdNr ungültig für $country';
    }
  }

  static String? _asString(Object? value) => value is String ? value : value?.toString();

  static num? _asNum(Object? value) {
    if (value is num) return value;
    return value is String ? num.tryParse(value) : null;
  }

  static int? _asInt(Object? value) {
    final number = _asNum(value);
    return number?.toInt();
  }

  static bool _asBool(Object? value) {
    if (value is bool) return value;
    if (value is num) return value != 0;
    return value == '1' || value == 'true';
  }

  static int _toCents(num value) => (value * 100).round();

  static num _fromCents(int value) => value / 100;

  static String _requiredText(Object? value, String field) {
    final text = _asString(value);
    if (text == null || text.trim().isEmpty) {
      throw KundenException('$field ist Pflicht');
    }
    return text;
  }

  static int _requiredIntValue(Object? value, String field) {
    final number = _asInt(value);
    if (number == null) throw KundenException('$field ist ungültig');
    return number;
  }

  static void _validateForeignTaxNumber(String? value) {
    if (value != null && value.length > 50) {
      throw const KundenException('Steuernummer Ausland darf höchstens 50 Zeichen enthalten');
    }
  }

  Future<String?> _customerReference(TransactionExecutor transaction, int id) async {
    final queries = <({String sql, String label})>[
      (sql: 'SELECT id, rechnungsnummer FROM rechnungen WHERE kunde_id = ? ORDER BY id LIMIT 1', label: 'Rechnung'),
      (sql: 'SELECT id FROM forderungen WHERE kunde_id = ? ORDER BY id LIMIT 1', label: 'Forderung'),
      (sql: 'SELECT id FROM mahnungen WHERE kunde_id = ? ORDER BY id LIMIT 1', label: 'Mahnung'),
      (sql: 'SELECT id, beleg_id FROM kunden_belege WHERE kunde_id = ? ORDER BY id LIMIT 1', label: 'Beleg'),
      (sql: 'SELECT id FROM kunden_lieferadressen WHERE kunde_id = ? ORDER BY id LIMIT 1', label: 'Lieferadresse'),
      (sql: 'SELECT id FROM rechnungsvorlagen WHERE kunde_id = ? ORDER BY id LIMIT 1', label: 'Rechnungsvorlage'),
    ];
    for (final query in queries) {
      final rows = await transaction.runSelect(query.sql, <Object?>[id]);
      if (rows.isEmpty) continue;
      final row = rows.single;
      final number = query.label == 'Rechnung'
          ? (_asString(row['rechnungsnummer']) ?? _asString(row['id']))
          : query.label == 'Beleg'
          ? (_asString(row['beleg_id']) ?? _asString(row['id']))
          : _asString(row['id']);
      return '${query.label} #$number';
    }
    return null;
  }

  static Future<void> _rollback(TransactionExecutor transaction) async {
    await transaction.rollback();
  }
}

class _ColumnDefinition {
  const _ColumnDefinition(this.name, this.definition);

  final String name;
  final String definition;
}

class _NoopTransactionUser extends QueryExecutorUser {
  @override
  int get schemaVersion => 0;

  @override
  Future<void> beforeOpen(QueryExecutor executor, OpeningDetails details) async {}
}
